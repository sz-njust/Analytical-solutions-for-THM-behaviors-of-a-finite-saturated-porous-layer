function lamda2=lamda2(s,xi)
global a alpha alphad betae cd eta G v h kpr kptr ktpr ktr kpz kptz ktpz ktz M md T0; 
lamda2=-((1./(4.*eta.*G.*(kptz.*ktpz - kpz.*ktz).*M.*md)).*((-alpha).*alphad.*cd.*kptz.*M.*s + 2.*betae.*cd.*eta.*G.*kptz.*M.*s + alphad.^2.*cd.*kpz.*M.*s + 2.*eta.*G.*ktz.*md.*s + 2.*cd.*eta.*G.*kpz.*M.*md.*s - ...
